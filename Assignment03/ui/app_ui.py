...
# Sidebar: check API health on every page load so the user always knows backend status
with st.sidebar:
    ...
    # Fetch model metadata dynamically from /info so it stays in sync with the backend
    ...
    try:
        ...
        if info_resp.status_code == 200:
            ...
            st.write(f"**CV Accuracy:** {info.get('cv_accuracy', 0):.1%}")
            st.write(f"**Features:** {info.get('n_features', '?')}")
            with st.expander("View all features"):
                for feat in info.get("features_expected", []):
                    st.write(f"• {feat}")
    except Exception:
...
def call_predict_api(payload: dict) -> dict:
    ...
    Wraps the HTTP call so the UI only ever sees success/failure,
    never a raw Python exception or traceback.
    ...
    try:
        ...
        return {
            ...
            "error": "Cannot connect to API. "
                     "Start it with: python -m uvicorn api.main:app --reload",
        }
    except requests.exceptions.Timeout:
        return {"success": False, "error": "The API took too long to respond. Try again."}
